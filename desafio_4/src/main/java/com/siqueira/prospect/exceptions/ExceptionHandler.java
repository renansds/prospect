...
public class ExceptionHandler {
	...
	public ResponseEntity<StandardError> objectNotFound(MethodArgumentTypeMismatchException e) {
		ValidationError error = new ValidationError(HttpStatus.BAD_REQUEST.value(), e.getMessage(), Instant.now().toEpochMilli());
	...
	@org.springframework.web.bind.annotation.ExceptionHandler(QueueEmptyException.class)
	public ResponseEntity<ValidationError> handleQueueEmptyException(QueueEmptyException ex) {
		ValidationError error = new ValidationError(HttpStatus.NO_CONTENT.value(), ex.getMessage(),
				Instant.now().toEpochMilli());
		return new ResponseEntity<>(error, HttpStatus.NO_CONTENT);
	}
	@org.springframework.web.bind.annotation.ExceptionHandler(Throwable.class)
	public ResponseEntity<StandardError> handleGlobalException(Throwable e) {
		StandardError standardError = new StandardError(HttpStatus.INTERNAL_SERVER_ERROR.value(), "Erro interno no servidor", Instant.now().toEpochMilli());
